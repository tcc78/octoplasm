...
public class GameCanvas implements InputProcessor {
    //=========================================================================
    //#region Fields
    ...
    /** TODO documentation */
    private final float CAMERAMOVESPEED = 20f;
    ...
    /** TODO: @Stephen Documentation */
    private float cameraMovedX;
    /** TODO: @Stephen Documentation */
    private float cameraMovedY;
    /**  */
     ...
     */
    public int getWidth() {
        return (viewport == null ) ? Gdx.graphics.getWidth() : (int) viewport.getWorldWidth();
    }
    ...
    public int getHeight() {
        return (viewport == null ) ? Gdx.graphics.getHeight() : (int) viewport.getWorldHeight();
    }
    ...
    /**
     * Sets the stage to be the input processor of the game.
     */
    ...
    public void resize(int width, int height) {
        ...
        Gdx.gl.glViewport(viewport.getScreenX(), viewport.getScreenY(), viewportWidth, viewportHeight);
        spriteBatch.getProjectionMatrix().setToOrtho2D(viewport.getScreenX(), viewport.getScreenY(), viewportWidth, viewportHeight);
     ...
     * TODO: Documentation
     */
    public void resetCameraToWindowSize(){
        camera.setToOrtho(false, getWidth(), getHeight());
        viewport.apply();
    }

    public void resetCameraToWorldPos() {
        viewport.apply(true);
    }

    //TODO: remove
    public void testBounds(){
//        System.out.println("camera: " + camera.position.x + "," + camera.position.y + " width = " +
//                camera.viewportWidth + " height = "+ camera.viewportHeight);
    }

    public Rectangle cameraBounds(){
        return new Rectangle(camera.position.x-camera.viewportWidth/2,camera.position.y-camera.viewportHeight/2,
    ...
    /**
     * moves the camera up up to the point where the viewspace will include area beyond ybound
     * @param ybound the furthest position along the y that can be seen by the camera
     */
    public void moveCameraUp(float ybound){
        if(camera.position.y + camera.viewportHeight/2 < ybound){
            camera.translate(0,CAMERAMOVESPEED);
            if(Gdx.input.isCursorCatched())
                Gdx.input.setCursorPosition(Gdx.input.getX(),
                        Gdx.input.getY() - (int)(CAMERAMOVESPEED * viewport.getScreenHeight()/viewport.getWorldHeight() + 0.5f));
            cameraMovedY = CAMERAMOVESPEED;
        }
        else camera.position.y = ybound - camera.viewportHeight/2;
//        System.out.println(ybound);
    }

    /**
     * moves the camera down up to the point where the viewspace will include area beyond ybound
     * @param ybound the furthest position along the y that can be seen by the camera
     */
    public void moveCameraDown(float ybound){
        if(camera.position.y - camera.viewportHeight/2 > ybound){
            camera.translate(0,-CAMERAMOVESPEED);
            if(Gdx.input.isCursorCatched())
                Gdx.input.setCursorPosition(Gdx.input.getX(),
    ...
    /**
     * moves the camera right up to the point where the viewspace will include area beyond xbound
     * @param xbound the furthest position along the x that can be seen by the camera
     */
    public void moveCameraRight(float xbound){
        if(camera.position.x + camera.viewportWidth/2 < xbound){
            camera.translate(CAMERAMOVESPEED,0);
            if(Gdx.input.isCursorCatched())
                Gdx.input.setCursorPosition(Gdx.input.getX() + (int)(CAMERAMOVESPEED * viewport.getScreenWidth()/viewport.getWorldWidth() + 0.5f),
                        Gdx.input.getY());
            cameraMovedX = CAMERAMOVESPEED;

        }
        else camera.position.x = xbound - camera.viewportWidth/2;
    }

    /**
     * moves the camera left up to the point where the viewspace will include area beyond xbound
     * @param xbound the furthest position along the x that can be seen by the camera
     */
    public void moveCameraLeft(float xbound){
        if(camera.position.x - camera.viewportWidth/2 > xbound){
            camera.translate(-CAMERAMOVESPEED,0);
            if(Gdx.input.isCursorCatched())
                Gdx.input.setCursorPosition(Gdx.input.getX() - (int)(CAMERAMOVESPEED * viewport.getScreenWidth()/viewport.getWorldWidth() + 0.5f),
     ...
     *
     *
     * @return Center position of the camera in screen coordinates
     */
    public Vector2 getCameraPosInScreen() {
        return cache.set(camera.position.x, camera.position.y);
    }

    public void setCameraPosInScreen(Vector2 position) {
        camera.position.x = position.x;
        camera.position.y = position.y;
    }

    /** TODO: @Stephen Documentation */
    public float getCameraMoveX() {
        return cameraMovedX;
    }

    /** TODO: @Stephen Documentation */
    public float getCameraMoveY() {
        return cameraMovedY;
    }
    //#endregion
    //=================================
...
//        Gdx.gl.glClearColor(0.39f, 0.58f, 0.93f, 1.0f);  // Homage to the XNA years
    ...
    public void draw(TextureRegion region, float ox, float oy,
                     float x, float y, float angle, float sx, float sy) {
        if (active != DrawPass.STANDARD) {
            Gdx.app.error("GameCanvas", "Cannot draw without active begin()", new IllegalStateException());
            return;
     ...
     * @param y The y-value
     */
    public void drawTextWithWidth(String text, BitmapFont font, float x, float y) {
        if (active != DrawPass.STANDARD) {
            Gdx.app.error("GameCanvas", "Cannot draw without active begin()", new IllegalStateException());
            return;
        }
    ...
    /**
     * Updates and draws all elements contained in the stage.
     *
     * @param delta The number of seconds since the last animation frame.
     */
    public void drawStage(float delta) {
        stage.act(delta);
//        stage.getViewport().apply(true);
//        for (Actor actor : stage.getActors()) {
//            actor.setPosition(camera.position.x - Gdx.graphics.getWidth()/2f,
...
//        }
    ...
    /**
     * Adds a new table to the stage.
     *
     * @param table The table to be added to the stage.
     */
    ...
    /**
     * Clears the stage of all actors.
     */
    ...
    public void drawSkeleton(Skeleton skeleton) {
        ...
        skeletonRenderer.draw(spriteBatch, skeleton);
    }
    //#endregion
    //=================================

    //=========================================================================
    //#region Input Processor Methods
    /** TODO: @Stephen Documentation */
    private Vector2 getCameraStageProjectionOffset() {
        return cache.set(-camera.position.x, camera.position.y)
                .sub(-camera.viewportWidth/2, camera.viewportHeight/2)
                .scl(viewport.getScreenWidth()/viewport.getWorldWidth(),
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        if (!disableUI) {
            if (Gdx.input.isCursorCatched()) {
                Vector2 tmp = getCameraStageProjectionOffset();
                return stage.touchDown(screenX + (int) tmp.x, screenY + (int) tmp.y, pointer, button);
            } else
    ...
    public boolean touchUp(int screenX, int screenY, int pointer, int button) {
        if (!disableUI) {
            if (Gdx.input.isCursorCatched()) {
                Vector2 tmp = getCameraStageProjectionOffset();
                return stage.touchUp(screenX + (int) tmp.x, screenY + (int) tmp.y, pointer, button);
            } else
    ...
    public boolean touchDragged(int screenX, int screenY, int pointer) {
        if (!disableUI) {
            if (Gdx.input.isCursorCatched()) {
                Vector2 tmp = getCameraStageProjectionOffset();
                return stage.touchDragged(screenX + (int) tmp.x, screenY + (int) tmp.y, pointer);
            } else
    ...
    public boolean mouseMoved(int screenX, int screenY) {
        if (!disableUI) {
            if (Gdx.input.isCursorCatched()) {
                Vector2 tmp = getCameraStageProjectionOffset();
                return stage.mouseMoved(screenX + (int) tmp.x, screenY + (int) tmp.y);
            } else
